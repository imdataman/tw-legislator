...
export default {
  ...
  mounted() {
    ...
    const ageScaleMobile = d3.scaleThreshold(
      [30, 40, 50, 60, 70],
      [
        HEIGHT / 5,
        (HEIGHT * 2) / 6,
    ...
    const partyScaleDesktop = d3.scaleOrdinal(
      [
        "民主進步黨",
        "中國國民黨",
        "台灣民眾黨",
      ...
      [
        LEFT_POSITION,
        RIGHT_POSITION,
        CENTER_POSITION_DESKTOP,
        CENTER_POSITION_DESKTOP,
        CENTER_POSITION_DESKTOP,
        CENTER_POSITION_DESKTOP
      ]
    );
    const partyScaleMobile = d3.scaleOrdinal(
      [
        "民主進步黨",
        "中國國民黨",
        "台灣民眾黨",
    ...
    const termScaleDesktop = d3.scaleThreshold(
      [2, 6],
      [LEFT_POSITION, CENTER_POSITION_DESKTOP, RIGHT_POSITION]
    );
    const termScaleMobile = d3.scaleThreshold(
      [2, 6],
      [UPPER_POSITION, CENTER_POSITION_MOBILE, LOWER_POSITION]
    );

    const genderScaleDesktop = d3.scaleOrdinal(
      ["男", "女"],
      [WIDTH / 6, RIGHT_POSITION]
    );
    const genderScaleMobile = d3.scaleOrdinal(
      ["男", "女"],
      [HEIGHT / 6, LOWER_POSITION]
    );
    ...
    const scrollScript = [
      ...
      },
      {
        ...
        text:
          "民進黨 <span>62</span> 席 國民黨 <span>38</span> 席 第三勢力 <span>13</span> 席"
      },
      {
        step: 2,
        desktop() {
        ...
        text:
          "友綠立委算入綠營</br>友藍立委算入藍營</br>綠營 <span>65</span> 席 藍營 <span>40</span> 席 中間 <span>8</span> 席"
      },
      {
        step: 3,
        desktop() {
        ...
        text:
          "依年齡分類</br><span>50</span> 世代人數最多</br>國民黨立委集中在 <span>60</span> 世代</br>最年輕和最年長都來自民進黨"
      },
      {
        step: 4,
        desktop() {
        ...
        text:
          "依照任期分類</br><span>38</span> 名立委是立院新鮮人</br><span>67</span> 名立委任期介於1-5屆</br><span>8</span> 名立委即將邁入第六屆以上"
      },
      {
        step: 5,
        desktop() {
          return d3
            .forceX(d => genderScaleDesktop(d.gender))
            .strength(STRENGTH);
        },
        mobile() {
        ...
        text:
          "依照性別分類</br><span>66</span> 名立委是男性</br><span>47</span> 名立委是女性</br>女性立委比率首度突破 <span>40%</span>"
      },
      {
        step: 6,
        desktop() {
        ...
        text:
          '以下讀者可自行探索資料</br>可選擇各種類別分類資料</br>點擊頭像可得基本資料</br><span id="byline">製作 <a href="https://github.com/imdataman" target="_blank">林佳賢</a> <a href="https://github.com/imdataman/tw-legislator" target="_blank">Github</a></span>'
      },
      {
        step: 7,
        desktop() {
    ...
    const forceX = d3.forceX(CENTER[0]).strength(0.5);

    const forceY = d3.forceY(CENTER[1]).strength(0.5);

    const simulation = d3.forceSimulation();

    if (MOBILE) {
      simulation
        .force("y", scrollScript[0].mobile())
        .force("x", forceX)
        .force("center", d3.forceCenter(CENTER[0], CENTER[1]))
        .force(
          "collide",
          d3
            .forceCollide(() => COLLIDE_RADIUS)
            .strength(STRENGTH)
            .iterations(100)
        );
    } else {
      simulation
        .force("x", scrollScript[0].desktop())
        .force("y", forceY)
        .force("center", d3.forceCenter(CENTER[0], CENTER[1]))
        .force(
          "collide",
    ...
    async function init() {
      ...
      if (MOBILE) {
        circles.on("click", d => {
          ...
          const imageCenter = [
            circles.filter(circle => circle.id == d.id).datum().x,
            circles.filter(circle => circle.id == d.id).datum().y
          ];

          tooltip
            .html(d.name + "</br>" + d.age + "歲</br>第" + d.term + "任")
            .style(
              "left",
              imageCenter[0] +
                CIRCLE_RADIUS_LARGE +
                (CIRCLE_RADIUS_LARGE - CIRCLE_RADIUS) +
                "px"
            )
      ...
      function ticked() {
        circles.attr("x", d => d.x).attr("y", d => d.y);
      }
